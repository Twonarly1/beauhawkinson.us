...
export function Typewriter({ text }: Props) {
    ...
    useEffect(() => {
        const intervalId = setInterval(() => {
            ...
            const shouldContinueWriting = !isErasing && textToRender.length !== subText.length
            const shouldStopWriting = !isErasing && textToRender.length === subText.length
            ...
            if (shouldContinueErasing) {
                setTextToRender((textToRender) => textToRender.slice(0, textToRender.length - 1))
