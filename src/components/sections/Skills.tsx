...
const Skills = () => {
    return (
        <div className="section">
            ...
            <div className="mt-12 grid max-w-5xl grid-cols-3 gap-6 transition-transform duration-500 sm:grid sm:grid-cols-5 md:grid md:grid-cols-7">
                {skills.map((skill: Skill, index: number) => (
                    <a
                        key={index}
                        className={`commonBordering relative p-4 dark:bg-slate-800 ${
                            skill.stack
                        ...
                        // href={skill.url}
                    >
                        <div className="peer relative mx-auto  h-10 w-10">
                            <Image
                                src={skill.src}
                                alt={skill.name}
                                objectFit="contain"
                        ...
                        <p className=" pointer-events-none absolute -left-0 -bottom-6  mx-auto w-full justify-center text-center text-sm transition-opacity peer-hover:opacity-100 [@media(hover:hover)]:opacity-0">
                            {skill.name}
                        </p>
                        {/* <p className="truncate pt-2 text-center text-[11px]">{skill.name}</p> */}
                    </a>
                ))}
            </div>
        </div>
